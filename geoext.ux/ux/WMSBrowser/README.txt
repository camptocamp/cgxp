Diff after updated to revision 2906

diff --git a/geoext.ux/ux/WMSBrowser/examples/WMSBrowser.html b/geoext.ux/ux/WMSBrowser/examples/WMSBrowser.html
index d30f07a..5742ef0 100644
--- a/geoext.ux/ux/WMSBrowser/examples/WMSBrowser.html
+++ b/geoext.ux/ux/WMSBrowser/examples/WMSBrowser.html
@@ -7,11 +7,11 @@
 
     <!-- Versions, Ext 3.1.0, GeoExt trunk, OpenLayers trunk -->
     <link rel="stylesheet" type="text/css"
-          href="../../../../ext/Ext/resources/css/ext-all.css" />
+          href="../../../../ext/resources/css/ext-all.css" />
     <link rel="stylesheet" type="text/css"
-          href="../../../../ext/Ext/examples/shared/examples.css" />
+          href="../../../../ext/examples/shared/examples.css" />
     <link rel="stylesheet" type="text/css"
-          href="../../../../ext/Ext/examples/ux/statusbar/css/statusbar.css" />
+          href="../../../../ext/examples/ux/statusbar/css/statusbar.css" />
     <link rel="stylesheet" type="text/css"
           href="../resources/css/WMSBrowser.css" />
   </head>
@@ -55,11 +55,11 @@
     </p>
 
     <script type="text/javascript"
-            src="../../../../ext/Ext/adapter/ext/ext-base.js"></script>
+            src="../../../../ext/adapter/ext/ext-base.js"></script>
     <script type="text/javascript"
-            src="../../../../ext/Ext/ext-all.js"></script>
+            src="../../../../ext/ext-all.js"></script>
     <script type="text/javascript"
-            src="../../../../ext/Ext/examples/ux/statusbar/StatusBar.js"></script>
+            src="../../../../ext/examples/ux/statusbar/StatusBar.js"></script>
     <script type="text/javascript"
             src="../../../../openlayers/lib/OpenLayers.js"></script>
     <!--script type="text/javascript"
